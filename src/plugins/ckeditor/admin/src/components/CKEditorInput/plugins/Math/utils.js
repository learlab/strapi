const {BalloonPanelView} = window.CKEDITOR;
...
export function isMathJaxVersion3( MathJax ){
	return (
...
export function isMathJaxVersion2( MathJax ){
	return (
...
function renderMathJax3( equation, element, display, cb) {
	...
	if ( promiseFunction != null ) {
		void promiseFunction( equation, { display } ).then( ( node ) => {
